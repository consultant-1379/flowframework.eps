...
import com.ericsson.component.aia.services.eps.builtin.components.mesa.integration.esper.smart.SmartSingletonUpdateListener;
...
public class SingletonListenerTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		esper = EPServiceProviderManager.getProvider("SingletonTest", config);
	}
	...
	private void loadTestModule() throws Exception {
		deploy("esper/sliding_window_for_singleton_test.epl");
		final Id coreId = new SimpleId(1);
		final Id ruleId = new SimpleId(1);
		esper.getEPAdministrator()
				.getStatement("sliding_window_singleton")
				.addListener(
						new SmartSingletonUpdateListener(
	...
	private final class SimpleViewListener implements ViewListener {
		...
		public void on(final View view) {
			...
			long check = view.getFirst().getRopId();
			for (final Event event : view.getAll()) {
				assertEquals(check, event.getRopId());
				// hardcoded event Rop IDs are 5 units apart
				check = check - 5;
			}
		}
